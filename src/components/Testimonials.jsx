import { assets, reviewInfo } from "../assets/assets"
 ...
 function Testimonials() {
    ...
    function slideRight() {
      const slider = document.getElementById('slider')
      if(window.innerWidth <= 490) {
        slider.scrollLeft += window.innerWidth - 23
      } else {
        slider.scrollLeft += 300
      }
    }
    ...
    return(
        <section id='testimonials' className="h-[80vh] flex items-center justify-center flex-col">
            <h1 className="text-3xl micro:text-4xl">Our <span className="text-green-600 underline">Testimonials</span></h1>
            <p className="text-gray-600 w-[90%] text-center text-sm micro:text-base mt-1">Check out reviews from some of our clients.</p>
            ...
            <div className="w-[90vw] mini:w-[75vw]">
                <div id='slider' className="flex gap-2 petite:gap-3 gallery overflow-x-scroll scroll scroll-smooth">
                    {reviewInfo.map((review, index) => {
                        return <div key={index} className="flex-shrink-0 flex gap-2 flex-col rounded border-[1px] border-black w-[100%] mini:w-[19rem] h-[27rem] p-2">
